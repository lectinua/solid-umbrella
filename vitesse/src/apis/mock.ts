...
export function mockPage(options: any, mockObj: (i: number) => any, page: Pagination = defaultPage, total = 100): Promise<any> {
  const list: Array<any> = []
  const start = (page.nowPage - 1) * page.cntPerPage
  for (let i = start; i < Math.min(start + page.cntPerPage, total); i++) {
    list.push(mockObj(i))
  }
